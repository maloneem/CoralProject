...
Number of Hexane ("nonpolar") Samples of Each Type: 
...
We examine the correlation between the bins we use as our explanatory variables.
...
Because we see that there are fairly high correlations between bins in several regions of both correlation plots, we have reason to believe that PCA might be appropriate to reduce dimensionality.
...
We see high (in absolute value) correlations between the first principal component and bins 31 and 17. We also see high correlation between bin 23 and the second principal component.
...
```{r clusteringtest, include=INCLUDE_SETTING}
...
colorCodes <- c(rep("blue", 23), rep("deeppink", 11))
...
colorCodes <- c(rep("blue", 18), rep("deeppink", 8))
```
...
We do not have enough samples to proceed with LDA for our hexane samples unless we omit some independent variables. 

```{r ldaHEX}
#HEX:
#leave-one-out cross validation
lda_cv_HEX = lda(as.factor(dfAllT_HEX[,3])~binned_matrix_HEX, CV=TRUE)
tab_lda_HEX = table(dfAllT_HEX[,3], lda_cv_HEX$class)
percents_lda_HEX = rbind(tab_lda_HEX[1, ]/sum(tab_lda_HEX[1, ]), tab_lda_HEX[2, ]/sum(tab_lda_HEX[2, ]), tab_lda_HEX[3, ]/sum(tab_lda_HEX[3,]))

dimnames(tab_lda_HEX) = list(c("Actual D", "Actual F", "Actual T"), c("Predicted D", "Predicted F", "Predicted T"))
dimnames(percents_lda_HEX) = list(c("Actual D", "Actual F", "Actual T"), c("Predicted D", "Predicted F", "Predicted T"))

kable(tab_lda_HEX)
kable(percents_lda_HEX, digits=3, caption="Proportion Actual vs. Predicted (HEX)")

lda_info_HEX = lda(as.factor(dfAllT_HEX[,3])~binned_matrix_HEX, CV=FALSE)
kable(lda_info_HEX$scaling, caption="Linear Discriminant Coefficient Matrix (HEX)") #Linear discriminant coefficient 

lda_info_HEX_p = predict(lda_info_HEX,as.data.frame(binned_matrix_HEX))
lda_info_HEX_pclass = predict(lda_info_HEX)$class
par(mar=c(5,4,4,4))
v = as.numeric(lda_info_HEX_pclass)
v[which(v==1)] =  "deeppink"   #D
v[which(v==2)] =  "blue"        #F
v[which(v==3)] =  "grey"       #Trochi

plot(lda_info_HEX_p$x[,1],pch=16, col = v,main = "LDA - 22 Second Bin Width",xlab="df1",ylab="df2") #Plot the linear discriminants
legend("bottomleft",legend=c("S. glaucum clade D","S. glaucum clade F","S. trocheliophorum"),col=c("deeppink",12,"grey"), pch=16,bty='o')
...
* http://maths-people.anu.edu.au/~johnm/courses/dm/math3346/2008/pdf/r-exercisesVI.pdf
    + 
* http://www.originlab.com/doc/Origin-Help/Discriminant-Analysis
    + Appropriate data for LDA
    + Assumptionsof LDA and how to verify